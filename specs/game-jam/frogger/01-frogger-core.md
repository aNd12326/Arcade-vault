...
### Decisiones tomadas durante la implementación (2026-06-07)

- **Cambio: `color = 'green'` en lugar de `'lime'`** — el contrato de plataforma (`CLAUDE.md`) restringe `color` al conjunto `cyan | magenta | yellow | green`, que mapea a clases de acento CSS. `lime` no tiene estilo de acento y la card se renderizaría sin color. `green` es el más cercano al tema rana y reutiliza el acento existente (igual que Snake). Confirmado con el usuario.

- **Cambio: patrón de plataforma (engine vanilla + ruta genérica) en lugar de componente React autocontenido + play-page dedicada** — el spec describía un único `FroggerGame.tsx` con el game loop dentro de React y una `app/games/frogger/play/page.tsx` propia con su modal de guardado. El repo real migró al patrón canónico: engine vanilla `public/games/<id>/game.js` (emite `av:*`, expone `window.<ID>`) + wrapper fino `app/_games/<id>/<PascalId>Canvas.tsx` + registro en `GAME_ENGINES`, jugado por la ruta genérica `app/games/[id]/play` cuyo `GamePlayerClient` ya gestiona HUD, pausa, modal game-over, `localStorage` (`av_player_name`) e INSERT en Supabase de forma genérica. Se implementó así para mantener coherencia con Asteroids/Tetris/Arkanoid/Snake (el propio spec exigía "igual que las play-pages de los demás juegos") y evitar duplicar `GamePlayerClient`. Toda la lógica `av:*` y de guardado del spec se conserva, solo cambia dónde vive. Confirmado con el usuario.

  Mapeo de archivos respecto al spec literal:
  - `components/games/FroggerGame.tsx` → `public/games/frogger/game.js` (engine) + `app/_games/frogger/FroggerCanvas.tsx` (wrapper).
  - `app/games/frogger/play/page.tsx` (paso 8) → ruta genérica existente `app/games/[id]/play/page.tsx` + `GamePlayerClient` (sin archivo nuevo).
  - Canvas: `640 × 560` (16 × 14 celdas de 40 px), según el data model del spec.
