...
// Click/espacio: reinicia si terminó, avanza nivel si completado, si no lanza la bola
...
document.getElementById( 'btn-mute' ).addEventListener( 'click', () => {
  muted = !muted;
  document.getElementById( 'btn-mute' ).textContent = muted ? '🔇 Mute' : '🔊 Sonido';
} );
...
function update( now ) {
  ...
  if ( game.state === 'won' || game.state === 'lost' || game.state === 'level_complete' ) return;
  ...
  for ( const b of blocks ) {
    ...
    if (
      ...
      playSound( 'break' );
      explosions.push( { color: b.color, x: b.x, y: b.y, w: b.w, h: b.h, start: now } );
      if ( blocks.every( ( bl ) => !bl.alive ) ) {
        game.state = game.level < 2 ? 'level_complete' : 'won';
      }
      break;
    }
  }
...
function draw( now ) {
  ...
  for ( const e of explosions ) {
    const frame = Math.min( Math.floor( ( now - e.start ) / ( EXPLOSION_DURATION / 4 ) ), 3 );
  ...
  if ( game.state === 'won' || game.state === 'lost' ) drawOverlay();
...
function drawOverlay() {
  ...
  const msg = game.state === 'won' ? '¡Ganaste!' : 'Game Over';
  ctx.fillText( msg, W / 2, H / 2 - 40 );

  ctx.font = '20px monospace';
  ctx.fillText( 'Score: ' + game.score, W / 2, H / 2 + 10 );
  ctx.fillText( 'Click o Espacio para reiniciar', W / 2, H / 2 + 50 );
}
...
function drawLevelCompleteOverlay() {
  ...
  ctx.font = '48px monospace';
  ctx.fillText( 'Nivel ' + ( game.level + 1 ) + ' completado', W / 2, H / 2 - 40 );

  ctx.font = '20px monospace';
  ctx.fillText( 'Score: ' + game.score, W / 2, H / 2 + 10 );
  ctx.fillText( 'Click o Espacio para continuar', W / 2, H / 2 + 50 );
}
